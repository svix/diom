...
fn bench_cache<'a, M: Measurement>(ctx: BenchmarkContext, group: &mut BenchmarkGroup<'a, M>) {
            ...
            |(key, val)| {
                rt.block_on(async {
                    std::hint::black_box(
                        client
                            .cache()
                            .set(key, CacheSetIn::new(val, Duration::from_secs(60))),
                    )
                    .await
                    .unwrap();
                })
            ...
            |(key, val)| {
                rt.block_on(async {
                    std::hint::black_box(
                        client
                            .cache()
                            .set(key, CacheSetIn::new(val, Duration::from_millis(1))),
                    )
                    .await
                    .unwrap();
                })
            ...
            |(k, v)| {
                rt.block_on(async {
                    std::hint::black_box(
                        client
                            .cache()
                            .set(k, CacheSetIn::new(v, Duration::from_secs(60))),
                    )
                    .await
                    .unwrap();
                })
    ...
    rt.block_on(async {
        std::hint::black_box(client.cache().set(
            test_key.clone(),
            CacheSetIn::new(test_val.clone(), Duration::from_secs(60)),
        ))
        .await
        .unwrap();
    });
            ...
            |(key1, key2, val)| {
                rt.block_on(async {
                    std::hint::black_box(
                        client
                            .cache()
                            .set(key1, CacheSetIn::new(val, Duration::from_secs(60))),
                    )
                    .await
                    .unwrap();
