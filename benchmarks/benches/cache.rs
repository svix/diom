...
fn bench_cache<'a, M: Measurement>(ctx: BenchmarkContext, group: &mut BenchmarkGroup<'a, M>) {
    ...
    group.bench_function("cache_set_fixed_high_ttl", |b| {
        b.iter_batched(
            ...
            |(key, val)| {
                rt.block_on(async {
                    std::hint::black_box(
                        client.cache().set(CacheSetIn::new(key, 60_000, val), None),
                    )
                    .await
                    .unwrap();
                })
    ...
    group.bench_function("cache_set_fixed_low_ttl", |b| {
        b.iter_batched(
            ...
            |(key, val)| {
                rt.block_on(async {
                    std::hint::black_box(client.cache().set(CacheSetIn::new(key, 1, val), None))
                        .await
                        .unwrap();
                })
            },
    ...
    group.bench_function("cache_set_random_small", |b| {
        b.iter_batched(
            ...
            |(k, v)| {
                rt.block_on(async {
                    std::hint::black_box(client.cache().set(CacheSetIn::new(k, 60_000, v), None))
                        .await
                        .unwrap();
                })
            },
    ...
    rt.block_on(async {
        std::hint::black_box(client.cache().set(
            CacheSetIn::new(test_key.clone(), 60_000, test_val.clone()),
            None,
        ))
        .await
        .unwrap();
    ...
    group.bench_function("cache_get", |b| {
        b.iter_batched(
            ...
            |key| {
                rt.block_on(async {
                    std::hint::black_box(client.cache().get(CacheGetIn::new(key), None))
                        .await
                        .unwrap();
                })
            },
    ...
    group.bench_function("cache_set_delete", |b| {
        b.iter_batched(
            ...
            |(key1, key2, val)| {
                rt.block_on(async {
                    std::hint::black_box(
                        client.cache().set(CacheSetIn::new(key1, 60_000, val), None),
                    )
                    .await
                    .unwrap();
                    std::hint::black_box(client.cache().delete(CacheDeleteIn::new(key2), None))
                        .await
                        .unwrap();
                })
            },
