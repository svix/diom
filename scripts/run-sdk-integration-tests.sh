...
BUILD=true
BINARY="$ROOT_DIR/target/release/coyote-server"
SDK_ARGS=()
...
trap '"$SCRIPT_DIR/stop-coyote-server.sh"' EXIT

if [[ "$BUILD" == true ]]; then
    echo "==> Building server binary..."
    (cd "$ROOT_DIR" && cargo build --release --package coyote-server --bin coyote-server)
fi

"$SCRIPT_DIR/start-coyote-server.sh" "$BINARY"
