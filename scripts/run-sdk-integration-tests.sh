...
parse_java() {
    local output="$1"
    local xml_dir="$ROOT_DIR/z-clients/java/build/test-results/integrationTest"
    # Parse JUnit XML reports generated by Gradle
    # Self-closing <testcase .../> = pass, otherwise it contains <failure> or <error>
    if [[ -d "$xml_dir" ]]; then
        for xml_file in "$xml_dir"/*.xml; do
            [[ -f "$xml_file" ]] || continue
            # Extract failures count from testsuite element
            local fail_names
            fail_names=$(grep -oP '<testcase name="\K[^"]+(?="[^/]*>)' "$xml_file" 2>/dev/null | sed 's/()$//' || true)
            local all_names
            all_names=$(grep -oP '<testcase name="\K[^"]+' "$xml_file" | sed 's/()$//')
            for name in $all_names; do
                if [[ -n "$fail_names" ]] && echo "$fail_names" | grep -qx "$name"; then
                    echo "FAIL $name"
                else
                    echo "PASS $name"
                fi
