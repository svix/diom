...
#   COYOTE_TOKEN       Auth token for the running server (has a default)
#   COYOTE_SERVER_URL  Server URL (default: http://localhost:8050)
...
run_python() {
    ...
    (cd "$ROOT_DIR/z-clients/python" && COYOTE_INTEGRATION=1 uv run --group dev pytest tests/ -v)
}
