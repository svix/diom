...
fn start_coyote_with_operational_webhooks(
    ...
    let jh = tokio::spawn(coyote::run_with_prefix(
...
async fn test_endpoint_create_update_and_delete() {
    ...
    let (client_regular, client_op, org_id, _jh) = start_coyote_with_operational_webhooks(cfg);
...
async fn test_operational_webhooks_event_types_exist() {
    let cfg = get_default_test_config();
    let (_, client_op, _, _jh) = start_coyote_with_operational_webhooks(cfg);
