...
pub fn get_default_test_config() -> ConfigurationInner {
    ...
    let cfg = coyote::cfg::load().unwrap();

    cfg.as_ref().clone()
}

pub async fn start_coyote() -> (TestClient, tokio::task::JoinHandle<()>) {
    start_coyote_with_cfg(&get_default_test_config()).await
}

pub async fn start_coyote_with_cfg(
    cfg: &ConfigurationInner,
) -> (TestClient, tokio::task::JoinHandle<()>) {
    start_coyote_with_cfg_and_org_id(cfg, OrganizationId::new(None, None)).await
}

pub async fn start_coyote_with_cfg_and_org_id(
    cfg: &ConfigurationInner,
    org_id: OrganizationId,
) -> (TestClient, tokio::task::JoinHandle<()>) {
    let prefix = svix_ksuid::Ksuid::new(None, None).to_string();
    start_coyote_with_cfg_and_org_id_and_prefix(cfg, org_id, prefix).await
}

pub async fn start_coyote_with_cfg_and_org_id_and_prefix(
    cfg: &ConfigurationInner,
    org_id: OrganizationId,
    prefix: String,
) -> (TestClient, tokio::task::JoinHandle<()>) {
    ...
    let jh = tokio::spawn(
        coyote::run_with_prefix(Some(prefix), cfg, Some(listener))
