...
use crate::utils::{
    ...
    get_default_test_config, start_coyote,
};
...
async fn test_uid_across_users() {
    let (mut client, _jh) = start_coyote().await;
    ...
    // N.b. previously we made a 2nd call to `start_coyote()` just to create a 2nd client with
    // a fresh token.
