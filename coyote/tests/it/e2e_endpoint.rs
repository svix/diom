...
use crate::utils::{
    ...
    get_default_test_config, start_coyote, start_coyote_with_cfg,
    start_coyote_with_cfg_and_org_id, TestClient, TestReceiver,
};
...
async fn test_create() {
    let (client, _jh) = start_coyote().await;
...
async fn test_recovery_expected_retry_counts() {
    ...
    let (client, _jh) = start_coyote_with_cfg(&cfg).await;
...
async fn test_endpoint_secret_encryption() {
    ...
    let (client, jh) = start_coyote_with_cfg_and_org_id(&cfg, org_id.clone()).await;
    let app_id = create_test_app(&client, "app1").await.unwrap().id;
    ...
    let cfg = get_default_test_config();
    let (client, _jh) = start_coyote_with_cfg_and_org_id(&cfg, org_id.clone()).await;
    client
        .get::<IgnoredAny>(
            &format!("api/v1/app/{app_id}/endpoint/{}/secret/", ep.id),
...
async fn test_endpoint_stats() {
    ...
    let last_msg_time = {
        ...
        let cfg = get_default_test_config();
        let db = Arc::new(cfg);
        let db = coyote::db::init_db(&db).await;
...
async fn test_endpoint_header_key_capitalization() {
    let (client, _jk) = start_coyote().await;
