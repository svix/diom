...
///
/// For fields that are structs you intend to evolve in the future, used the `#[nested]` flag
/// so the field is stored in a way that allows adding sub-fields in a backwards and forwards compatible way.
#[proc_macro_derive(PersistableVersioned, attributes(since, versioned, nested))]
