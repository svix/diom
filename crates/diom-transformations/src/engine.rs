...
impl From<ScriptError> for diom_error::Error {
    fn from(err: ScriptError) -> Self {
        match err {
            ...
            | ScriptError::NoOutput => diom_error::Error::bad_request("content_validation", err),
            ScriptError::ExecutionException { .. } | ScriptError::ProcessTimeout => {
                diom_error::Error::bad_request("execution_timeout", err)
            }
            ScriptError::OutOfMemory => diom_error::Error::bad_request("out_of_memory", err),
        }
    }
}
