...
pub(crate) const OUTPUTS: &[OutputDirectory] = &[
    OutputDirectory::unmanaged_dir("z-clients/python/diom", &["python/summary.py.jinja"]),
    OutputDirectory::managed_dir(
        "z-clients/python/diom/apis",
        &[
...
pub(crate) async fn generate(api: &Api) -> anyhow::Result<()> {
    ...
    ContainerizedFormatter {
        ...
        cmd: &[
            "sh",
            "-c",
            "ruff check --fix /z-clients/python/diom && ruff format /z-clients/python/diom",
        ],
    }
    .run()
    .await?;
