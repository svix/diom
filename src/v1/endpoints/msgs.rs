...
impl RequestInput for SvixPollerCreateIn {
    fn access_metadata(&self) -> AccessMetadata<'_> {
        msgs_metadata(self.namespace.as_ref(), &self.topic, "svix_poller.create")
    }
}
...
impl RequestInput for SvixPollerDeleteIn {
    fn access_metadata(&self) -> AccessMetadata<'_> {
        msgs_metadata(self.namespace.as_ref(), &self.topic, "svix_poller.delete")
    }
}
...
#[aide_annotate(op_id = "v1.msgs.svix-poller.create")]
async fn svix_poller_create(
    State(state): State<AppState>,
    Extension(repl): Extension<RaftState>,
    MsgPackOrJson(data): MsgPackOrJson<SvixPollerCreateIn>,
) -> Result<MsgPackOrJson<SvixPollerCreateOut>> {
    diom_core::svix_client::validate_autoconfig_token(&data.token)
        .map_err(|_| Error::bad_request("invalid_token", "invalid autoconfig token"))?;
    ...
    let operation =
        SvixPollerCreateOperation::new(namespace.id, data.topic, data.poller_id, data.token);
    let response = repl.client_write(operation).await.or_internal_error()?.0?;

    Ok(MsgPackOrJson(SvixPollerCreateOut {
...
#[aide_annotate(op_id = "v1.msgs.svix-poller.delete")]
async fn svix_poller_delete(
    State(state): State<AppState>,
    Extension(repl): Extension<RaftState>,
    MsgPackOrJson(data): MsgPackOrJson<SvixPollerDeleteIn>,
) -> Result<MsgPackOrJson<SvixPollerDeleteOut>> {
    let namespace: MsgsNamespace = state
        .namespace_state
        .fetch_namespace(data.namespace.as_ref())?
        .ok_or_not_found("namespace")?;

    let operation = SvixPollerDeleteOperation::new(namespace.id, data.topic, data.poller_id);
    let response = repl.client_write(operation).await.or_internal_error()?.0?;

    Ok(MsgPackOrJson(SvixPollerDeleteOut {
        topic: response.topic,
        poller_id: response.poller_id,
...
impl RequestInput for SvixPollerListIn {
    fn access_metadata(&self) -> AccessMetadata<'_> {
        msgs_metadata(self.namespace.as_ref(), &self.topic, "svix_poller.list")
    }
}
...
impl ListResponseItem for SvixPollerOut {
    fn id(&self) -> String {
        self.poller_id.clone()
    }
}

impl From<SvixPollerListItem> for SvixPollerOut {
    fn from(item: SvixPollerListItem) -> Self {
        Self {
            topic: item.topic,
...
#[aide_annotate(op_id = "v1.msgs.svix-poller.list")]
async fn svix_poller_list(
    State(state): State<AppState>,
    Extension(repl): Extension<RaftState>,
    MsgPackOrJson(data): MsgPackOrJson<SvixPollerListIn>,
) -> Result<MsgPackOrJson<ListResponse<SvixPollerOut>>> {
    repl.wait_linearizable().await.or_internal_error()?;
    ...
    let items = items.into_iter().map(SvixPollerOut::from).collect();
...
pub fn router() -> ApiRouter<AppState> {
    ...
    ApiRouter::new()
        ...
        .api_route_with(
            svix_poller_create_path,
            post_with(svix_poller_create, svix_poller_create_operation),
            &tag,
        )
        .api_route_with(
            svix_poller_delete_path,
            post_with(svix_poller_delete, svix_poller_delete_operation),
            &tag,
        )
        .api_route_with(
