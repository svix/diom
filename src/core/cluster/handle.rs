...
#[derive(Debug, Clone)]
pub(crate) enum DiomErrorOrForwardToLeader {
    Error(diom_operations::OperationError),
    ForwardToLeader {
        leader_id: NodeId,
        leader_node: Node,
    },
...
impl DiomErrorOrForwardToLeader {
    #[track_caller]
    fn internal(e: impl fmt::Display) -> Self {
        Self::Error(diom_operations::OperationError::from(
            diom_error::Error::internal(e),
        ))
    }
}

impl From<diom_error::Error> for DiomErrorOrForwardToLeader {
    fn from(value: diom_error::Error) -> Self {
        Self::Error(value.into())
    }
}

impl From<RaftError<NodeId, ClientWriteError<NodeId, Node>>> for DiomErrorOrForwardToLeader {
...
impl RaftState {
    ...
    async fn guard_leader(&self) -> Result<(), DiomErrorOrForwardToLeader> {
        let Some(leader_id) = self.raft.current_leader().await else {
            return Err(DiomErrorOrForwardToLeader::internal(
        ...
        if leader_id != self.node_id {
            let Some(leader_node) = self
                ...
                return Err(DiomErrorOrForwardToLeader::from(
                    diom_error::Error::internal("unable to determine leader"),
                ));
            };
            return Err(DiomErrorOrForwardToLeader::ForwardToLeader {
                leader_id,
                leader_node,
            });
        }
    ...
    async fn remove_node_inner(&self, node_id: NodeId) -> Result<(), DiomErrorOrForwardToLeader> {
