...
impl RaftState {
    ...
    pub async fn client_write_background<O>(
        &self,
        op: O,
    ) -> diom_operations::BackgroundResult<O::Response>
    where
        ...
        O::Response: TryFrom<Response>,
    {
        ...
        let response = match self.raft.client_write(request.clone()).await {
            ...
            Err(err) => {
                if err.forward_to_leader().is_some() {
                    return Err(diom_operations::BackgroundError::NotLeader);
                } else {
                    return Err(diom_operations::BackgroundError::Other(
        ...
        let Ok(resp) = response.try_into() else {
            return Err(diom_operations::BackgroundError::Other(
                diom_error::Error::generic("invalid response type"),
            ));
        };
        Ok(resp)
    }
