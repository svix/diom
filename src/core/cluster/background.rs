...
impl CanBeForwardToLeader for diom_operations::BackgroundError {
    fn is_forward_to_leader_err(&self) -> bool {
...
impl BackgroundJob for KvBackground {
    async fn run_on_leader(self) -> diom_operations::BackgroundResult<()> {
        ...
        diom_kv::worker(
            store,
            async move |message: diom_kv::operations::KvOperation| {
...
impl BackgroundJob for CacheBackground {
    async fn run_on_leader(self) -> diom_operations::BackgroundResult<()> {
        ...
        diom_cache::worker(
            store,
            async move |message: diom_cache::operations::CacheOperation| {
                self.handle.client_write_background(message).await
            },
        )
        .await
...
impl BackgroundJob for IdempotencyBackground {
    async fn run_on_leader(self) -> diom_operations::BackgroundResult<()> {
        ...
        diom_idempotency::worker(
            store,
            async move |message: diom_idempotency::operations::IdempotencyOperation| {
                self.handle.client_write_background(message).await
            },
        )
        .await
...
struct BackgroundJobRunner {
    jobs: JoinSet<diom_operations::BackgroundResult<()>>,
}
