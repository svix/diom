...
- [Auth tokens](#auth-tokens): tokens are how users authenticate with Coyote. Each token has a role associated with it, which is what the token authenticates to.
