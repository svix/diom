...
Coyote offers granular access control to let you configure the access control exactly as you need it.
...
- [Role](#role): each action on Coyote is associated with a specific role.
...
- [Access rules](#access-rules): rules describe what operations are allowed or disallowed for a groups of resources.
- [Auth tokens](#auth-tokens): tokens are how users authenticate with Coyote. Each token has a role associated with it, which is what the token authenticate to.
  - Tokens may have additional context that can then be used in path replacements and in conditionals.


### Role

When you authenticate to Coyote, you authenticate to a particular role. That role is associated with the level of access you have and is required for authorization.
...
The policies is a list of policies, which are explained in the next section.
...
- Globbing is supported
  ...
  - Globbing is not currently supported for any other combination. For example, there are not supported: `cache::f*o/bar`, `cache::*o/bar`, and `cache::*/bar`.
...
Auth tokens are what Coyote clients use in order to authenticate against a Coyote server and assume a specific role. Therefore each auth token has exactly one role associated with it, but each role can have many auth tokens associated with it.
...
- Creating them in Coyote using an API.
- Using JWTs (coming soon).

Creating a new token using the API looks something like this:
```
coyote admin auth_token create '{"name": "token-name", "role": "frontend-role", "context": {"user": "their-user-id"}}'
# Returns:
{
    "id": "tok_123",
    "token": "sk_1234",
...
You can create roles, policies, and auth tokens using the Coyote API. However, you need an auth token in order to interact with the API.

To solve that, Coyote lets you configure an admin auth token in the configuration file, which you can use to create policies, roles, and tokens. The admin auth token assumes the role `admin` which has access to everything on the cluster.

To configure an admin auth token set the `COYOTE_ADMIN_TOKEN` environment variable to you wanted token, for example: `zpsNuhPMeMbg9q4cG2ZsJn`, and then you'll be able to use it for authenticating to that specific node. It's recommended that you only use the admin auth token for bootstrapping, and remove the configuration once done.
...
```bash
coyote admin auth-policy upsert '
    {
        "id": "my-policy",
        "description": "My first policy.",
        "rules": [
...
```bash
coyote admin auth_token create '
    {
        "name": "some-token",
        "role": "my-role",
...
First they would create a policy and a role (or we can have a default one, at least a policy) that looks something like this:
...
First they would create a role (or we can have a default one) that looks something like this:
