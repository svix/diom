...
Storing a value in the cache requires setting a key (string), a value (byte array), and the wanted time-to-live (TTL) for the value before it's evicted.
...
client.cache().set(
    ...
    CacheSetIn::new(value, Duration::from_secs(10))
)
.await?;
```
