...
Receiving messages is done using the `diom.msgs.fifo.receive` API as follows:

```js
diom.msgs.fifo.receive(
...
You can configure the behavior of consumer groups for a specific topic using the `diom.msgs.fifo.configure` API as follows:

```js
diom.msgs.fifo.configure(
    topic="topic-123",
    consumer_group="group1",
